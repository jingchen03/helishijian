...
def getDistance(start, end):
    AK = '2hIdnGXn2sxdv7H2L5NfGzbrA1Sx6mRZ'
...
# 建一个列表把一个一个搞太难了
h = [car_22271, car_24272, car_26454, car_36805]
hdist = {'car_22271': 0, 'car_24272': 0, 'car_26454': 0, 'car_36805': 0}  # 字典后续存入距离
for i in h:
...
for j in h:
    ...
    for m in range(1, li):  # 这里m的值是从1到li，所以不会超出索引的
        start = str(j.iloc[m]["Lat"]) + ',' + str(j.iloc[m]['Lng'])  # 读取表格中的经纬度并且规范格式
        end = str(j.iloc[m + 1]["Lat"]) + ',' + str(j.iloc[m + 1]['Lng'])  # 调取上一个的下一行（最大到最后一行）的经纬度并且规范格式
        dist = getDistance(start, end)
        hdist[hzifuchuan] = int(hdist.get(hzifuchuan)) + dist
        end111 = time.perf_counter()
        print('----------Running time: {}s----------api算出距离为{}m'.format(int(end111 - start111),
